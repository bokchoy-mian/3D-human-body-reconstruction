...
class Joint:
  def __init__(self, name, direction, length, axis, dof, limits):
    self.name = name#骨骼名称
    self.direction = np.matrix(direction)#骨骼初始方向，从父节点到该节点，全局坐标系下的
    self.length = length#骨骼长度
    axis = np.deg2rad(axis)#绑定于该节点的“初始局部坐标系”，重要
    self.C = np.matrix(transforms3d.euler.euler2mat(*axis))
    self.Cinv = np.linalg.inv(self.C)
    self.limits = np.zeros([3, 2])
    self.movable = len(dof) == 0
    #dof amc文件里面的旋转角度，用以后续坐标的计算
    for lm, nm in zip(limits, dof):
      if nm == 'rx':
        ...
        self.limits[2] = lm
    self.parent = None
    self.children = []#该骨骼的儿子节点id，但可能有多个儿子
  ...
  def set_motion(self, motion):
    if self.name == 'root':
      ...
      self.matrix = self.C * np.matrix(transforms3d.euler.euler2mat(*rotation)) * self.Cinv#transforms3d.euler.euler2mat将欧拉角转为旋转矩阵
