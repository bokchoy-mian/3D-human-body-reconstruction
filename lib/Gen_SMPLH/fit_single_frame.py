...
from lib.Gen_SMPLH.optimizers import optim_factory

from lib.Gen_SMPLH import fitting
...
def fit_single_frame(img,keypoints,
                     ...
                     jaw_prior,
                     angle_prior,
                     out_pkl_path='out.pkl',
                     out_obj_path='out.obj',
                     out_rendimg_path='overlay.png',
                     loss_type='smplify',
                     use_cuda=True,
    ...
    init_t = fitting.guess_init(body_model, gt_joints, edge_indices,
                                ...
                                model_type=kwargs.get('model_type', 'smplh'),
    ...
    with fitting.FittingMonitor(
        ...
        for or_idx, orient in enumerate(tqdm(orientations, desc='Orientation')):
            ...
            results.append({'loss': final_loss_val,
                            'result': result})
        with open (out_pkl_path, 'wb') as result_file:
            if len (results) > 1:
    ...
    if save_meshes :
        ...
        model_output = body_model(betas=None,return_verts=True, body_pose=body_pose,return_full_pose=True)
    ...
    wrist_pose = torch.zeros ([body_pose.shape[0], 6],
                              dtype=body_pose.dtype,
                              device=body_pose.device)
    smpl_pose = torch.cat ([body_model.global_orient,body_pose, wrist_pose], dim=1).detach().cpu().numpy()

    smpl_result={'spml_shape':result['betas'][0],'spml_pose':smpl_pose[0],
                 'camera_fx' : focal_length,
                 'camera_center' : camera_center,
                 'camera_rotation':result['camera_rotation'][0],
                 'camera_translation':result['camera_translation'][0],
                 }


    # smplh_pose = torch.cat ([body_model.global_orient, body_pose, lhand_pose,rhand_pose], dim=1).detach ().cpu ().numpy ()
    smplh_pose =model_output.full_pose.detach().cpu().numpy().squeeze()
    smplh_result = {'spmlh_shape': result['betas'][0], 'spmlh_pose': smplh_pose,
                   'camera_fx': focal_length,'camera_center': camera_center,
                   'camera_rotation': result['camera_rotation'][0],
                   'camera_translation': result['camera_translation'][0],
                   }
    return result,smplh_result